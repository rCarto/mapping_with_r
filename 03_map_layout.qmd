...
water <- st_read("data/khm.gpkg", "water")
```
...
#| fig-height: 3
mf_init(khm)
mf_map(land, add = T, col = "#f2efe9", border = "#f2efe9")
mf_map(water, add = T, col = "lightblue", border = 'lightblue', lwd = 1.5)
mf_map(khm, add = T, col = "#c2dcb6", border = "grey50")
mf_title("Cambodia")
```
:::
...
mf_map(land, add = T, col = "#f2efe9", border = NA)
mf_map(water, add = T, col = "lightblue", border = 'lightblue', lwd = 1.5)
mf_map(khm, 'sex_ratio',"choro", border = "grey50",
...
mf_map(khm, 'sex_ratio',"choro", breaks = bks, pal = pals,  border = "grey50",
       add = T, leg_pos = NA)
...
mf_map(khm, 'sex_ratio',"choro", breaks = bks, pal = pals, 
       leg_val_rnd = 3, add = T,  border = "grey50")
mf_inset_on(x = khm[khm$ADM1_EN=="Phnom Penh", ], cex = .3)
mf_init(khm[khm$ADM1_EN=="Phnom Penh", ], theme = my_theme)
mf_map(khm, 'sex_ratio',"choro", breaks = bks, pal = pals,  border = "grey50",
       add = T, leg_pos = NA)
...
mf_map(land,  col = "#f2efe9", border = NA, add = T)
mf_map(water, add = T, col = "lightblue", border = 'lightblue', lwd = 1.5)
mf_map(khm, add = T, col = "#c2dcb6", lwd =.7, border = "grey50")
...
mf_map(khm, c('T_TL', 'sex_ratio'),"prop_choro", inches = .2,
